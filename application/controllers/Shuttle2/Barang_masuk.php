...
class Barang_masuk extends CI_Controller {
	...
	{
		$data['isi'] = $this->M_shuttle2->view_barang_masuk();
		$h = $this->load->view('shuttle2/home_shuttle2', $data);
		$c['content'] = $this->load->view('shuttle2/barang_masuk/view', $h);
	}

	// Form tambah barang_masuk
	public function tambah_barang_masuk()
	{
		$data['kode'] = $this->M_shuttle2->id_barang_masuk();
		$data['get_jns'] = $this->M_shuttle2->get_idbarang();
		$h = $this->load->view('shuttle2/home_shuttle2', $data);
		$c['content'] = $this->load->view('shuttle2/barang_masuk/tambah', $h);
	}
	...
	public function barang_masuk_tambah()
	{
		...
		if($cek <= 0){
			...
			$this->M_shuttle2->simpan_barang_masuk($data1);
			...
			$this->M_shuttle2->simpan_stok_barang($data2);
			
			redirect('shuttle2/barang_masuk/view_barang_masuk','refresh');
		}else{
			echo '<script language="javascript">';
			echo 'alert("Maaf ID Transaksi Sudah Ada")';
			echo '</script>';
			echo '<script language="javascript">';
			echo 'window.location=("'.site_url('shuttle2/barang_masuk/tambah_barang_masuk/').'")';
			echo '</script>';
		}
	}
	...
	{
		$this->M_shuttle2->delete_barang_masuk($id);
	...
	{	
		$data['get_jns'] = $this->M_shuttle2->get_idbarang();
		$data['ubah_data'] = $this->M_shuttle2->view_edit_barang_masuk($id);		
		$c['content'] = $this->load->view('shuttle2/barang_masuk/edit', $data);
		$this->load->view('shuttle2/home_shuttle2', $c);
	}
	...
	public function barang_masuk_edit($id)
	{
			...
			$this->M_shuttle2->edit_barang_masuk($id,$data1);
