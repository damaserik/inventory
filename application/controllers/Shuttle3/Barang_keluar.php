...
class Barang_keluar extends CI_Controller {
	...
	{
		$data['isi'] = $this->M_shuttle3->view_barang_keluar();
		$h = $this->load->view('shuttle3/home_shuttle3', $data);
		$c['content'] = $this->load->view('shuttle3/barang_keluar/view', $h);
	}

	// Form tambah barang_keluar
	public function tambah_barang_keluar()
	{
		$data['kode'] = $this->M_shuttle3->id_barang_keluar();
		$data['get_jns'] = $this->M_shuttle3->get_idbarang();
		$data['get_msn'] = $this->M_shuttle3->get_idmesin();
		$data['get_kg'] = $this->M_shuttle3->get_idkagrup();
		$data['get_mt'] = $this->M_shuttle3->get_idmtc();
		$h = $this->load->view('shuttle3/home_shuttle3', $data);
		$c['content'] = $this->load->view('shuttle3/barang_keluar/tambah', $h);
	}
	...
	public function barang_keluar_tambah()
	{
		...
		$ketersedian_stok = $this->M_shuttle3->get_cek_ketersedian();
		...
		foreach ($ketersedian_stok as $row) {
				...
				echo '</script>';
				redirect('shuttle3/barang_keluar/tambah_barang_keluar/','refresh');
				}	
		}
		
		$stok_all = $this->M_shuttle3->jmlh_stok($id_barang);
		$fifo = $this->M_shuttle3->stok_urut_tgl($id_barang);
		...
		$result = $data->result();
			...
			foreach($result as $row) {
				...
				if($jml_keluar > 0){
					...
					$sql1 = "UPDATE stok_barang SET stok = '$update_stok', stok_out = '$update_tot' + stok_out WHERE id_barang = '$id_barang' AND id_bagian='Shtl3' AND tanggal = '$tgl'";
					...
					$this->M_shuttle3->simpan_dtl_barang_kel($data3);

					/*$this->M_shuttle3->update_stok_barang($id_barang,$update_stok,$update_tot,$tgl);*/
				}
			...
			// redirect('shuttle3/barang_keluar/view_barang_keluar','refresh');
			...
			$this->M_shuttle3->simpan_barang_keluar($data1);
			redirect('shuttle3/barang_keluar/view_barang_keluar','refresh');
	}
	...
	{
		// $this->M_shuttle3->delete_barang_keluar($id);
		// $this->M_shuttle3->delete_stok_barang($id);
		// redirect('shuttle3/barang_keluar/view_barang_keluar','refresh');
	}
	...
	function hapus_brngkeluar($id)
	{
		...
		$this->M_shuttle3->delete_barang_keluar($id);
		$this->M_shuttle3->delete_dtl_stok_barang($id);
		redirect('shuttle3/barang_keluar/view_barang_keluar','refresh');
	}
	...
	// {	
	// 	$data['get_jns'] = $this->M_shuttle3->get_idbarang();
	// 	$data['ubah_data'] = $this->M_shuttle3->view_edit_barang_keluar($id);		
	// 	$c['content'] = $this->load->view('shuttle3/barang_keluar/edit', $data);
	// 	$this->load->view('shuttle3/home_shuttle3', $c);
	// } 
	...
	// 			'id_bagian' => $this->input->post('id_bagian', TRUE)
	// 		);
	// 		$this->M_shuttle3->edit_barang_keluar($id,$data1);
	...
	// 			'id_bagian' => $this->input->post('id_bagian', TRUE)
	// 		);
	// 		$this->M_shuttle3->edit_stok_barang($id,$data2);
