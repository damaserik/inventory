...
class M_laporan_shuttle2 extends CI_Model {
	...
	public function laporan_stok_view($bulan,$tahun)
	{
		$query = $this->db->query("SELECT barang.`id_barang`,barang.`nama_barang`, SUM(barang_masuk.`stok_masuk`) AS NB_masuk, 
				...
				AND MONTH(stok_barang.tanggal)='$bulan' AND YEAR(stok_barang.tanggal)='$tahun'
				AND stok_barang.`id_bagian`='Shtl2'
				GROUP BY id_barang
				UNION ALL
				SELECT '' AS id_barang,
	...
	public function laporan_stok_harga($bulan,$tahun)
	{
		$query = $this->db->query("SELECT barang.`id_barang`,barang.`nama_barang`, SUM(barang_masuk.`stok_masuk`) AS NB_masuk, 
	...
	public function laporan_pemakaian_view($bulan,$tahun,$zona)
	{
		$query = $this->db->query("SELECT mesin.`zona`,mtc.`nama_mtc` AS mtc,mesin.`id_mesin` AS mc,barang_keluar.`tgl_keluar` AS tanggal,barang.`nama_barang`,stok_keluar AS jumlah_pakai,
				...
				AND MONTH(barang_keluar.`tgl_keluar`)='$bulan' AND YEAR(barang_keluar.`tgl_keluar`)='$tahun' AND barang_keluar.`id_bagian`='Shtl2'
