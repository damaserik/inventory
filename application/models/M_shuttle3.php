...
class M_shuttle3 extends CI_Model {
	...
	public function view_barang()
	{
		...
		WHERE barang.`id_jenis`=jenis.`id_jenis` AND barang.`id_fungsi`=fungsi.`id_fungsi` AND barang.id_bagian='Shtl3'
	...
	{
		$sql = "SELECT id_mesin,zona FROM mesin where id_bagian='Shtl3'";
		$data = $this->db->query($sql);
		return $data->result();
	}
	...
	{
		$sql = "SELECT id_kagrup,nama_kagrup FROM kagrup where id_bagian='Shtl3' order by id_kagrup asc";
		$data = $this->db->query($sql);
		return $data->result();
	}
	...
	{
		$sql = "SELECT id_mtc,nama_mtc FROM mtc where id_bagian='Shtl3' order by id_mtc asc";
		$data = $this->db->query($sql);
		return $data->result();
	}
	...
	public function view_barang_masuk()
	{
		$sql = "SELECT barang_masuk.`id_transaksi`,tgl_masuk,barang.`nama_barang`,stok_masuk,stok_barang.`harga`,total_harga_masuk
				...
				WHERE barang_masuk.`id_transaksi`=stok_barang.`id_transaksi` AND stok_barang.`id_barang`=barang.`id_barang` AND barang.`id_bagian`='Shtl3'
	...
	public function view_stok()
	{
		$sql = "SELECT barang.`id_barang`,barang.`nama_barang`,SUM(`stok`) AS stok
				...
				WHERE barang.`id_barang`=stok_barang.`id_barang` AND stok_barang.`id_bagian`='Shtl3'
	...
	{
		$sql = "SELECT * FROM barang_keluar as a join dtl_barang_keluar as b on a.`id_transaksi`=b.`id_transaksi`  join barang as c on b.`id_barang`=c.`id_barang` WHERE a.`id_bagian`='Shtl3' group by a.`id_transaksi` ORDER BY a.`tgl_keluar` desc";
		$data = $this->db->query($sql);
		return $data->result();
	}
