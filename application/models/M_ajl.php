...
class M_ajl extends CI_Model {
	...
	{
		$sql = "SELECT id_barang,nama_barang,status_barang,jenis,fungsi FROM barang,jenis,fungsi 
		WHERE barang.`id_jenis`=jenis.`id_jenis` AND barang.`id_fungsi`=fungsi.`id_fungsi` AND barang.id_bagian='Ajl'
	...
	{
		$sql = "SELECT id_mesin,zona FROM mesin where id_bagian='Ajl'";
		$data = $this->db->query($sql);
		return $data->result();
	}
	...
	{
		$sql = "SELECT id_kagrup,nama_kagrup FROM kagrup where id_bagian='Ajl' order by id_kagrup asc";
		$data = $this->db->query($sql);
		return $data->result();
	}
	...
	{
		$sql = "SELECT id_fungsi,fungsi FROM fungsi WHERE id_bagian='Ajl' order by id_fungsi asc";
		$data = $this->db->query($sql);
		return $data->result();
	}
	...
	{
		$sql = "SELECT id_jenis,jenis FROM jenis WHERE id_bagian='Ajl' order by id_jenis asc";
		$data = $this->db->query($sql);
		return $data->result();
	}
	...
	{
		$sql = "SELECT id_mtc,nama_mtc FROM mtc where id_bagian='Ajl' order by id_mtc asc";
		$data = $this->db->query($sql);
		return $data->result();
	}
	...
	public function view_barang_masuk()
	{
		$sql = "SELECT barang_masuk.`id_transaksi`,tgl_masuk,barang.`nama_barang`,stok_masuk,stok_barang.`harga`,total_harga_masuk
				...
				WHERE barang_masuk.`id_transaksi`=stok_barang.`id_transaksi` AND stok_barang.`id_barang`=barang.`id_barang` AND barang.`id_bagian`='Ajl'
	...
	public function view_stok()
	{
		$sql = "SELECT barang.`id_barang`,barang.`nama_barang`,SUM(`stok`) AS stok
				...
				WHERE barang.`id_barang`=stok_barang.`id_barang` AND stok_barang.`id_bagian`='Ajl'
	...
	{
		$sql = "SELECT * FROM barang_keluar as a join dtl_barang_keluar as b on a.`id_transaksi`=b.`id_transaksi`  join barang as c on b.`id_barang`=c.`id_barang` WHERE a.`id_bagian`='Ajl' group by a.`id_transaksi` ORDER BY a.`tgl_keluar` desc";
		$data = $this->db->query($sql);
		return $data->result();
	}
